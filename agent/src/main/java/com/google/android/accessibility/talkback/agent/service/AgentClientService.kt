    ...
    override fun onConnectionStateChanged(
        ...
        val statusText = when (state) {
            ConnectionManager.ConnectionState.CONNECTED -> {
                ...
                // Create skill poller lazily if not already created
                if (skillPoller == null) {
                    val agentManager = AgentManager.getInstanceOrNull()
                    val agent = agentManager?.getAgent()
                    android.util.Log.i(TAG, "Creating SkillPoller: agentManager=$agentManager, agent=$agent")
                    agent?.let {
                        skillPoller = SkillPoller(it)
                        android.util.Log.i(TAG, "SkillPoller created successfully")
                    } ?: android.util.Log.w(TAG, "Cannot create SkillPoller - agent is null")
                }
